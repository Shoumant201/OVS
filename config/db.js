...
const createTables = async () => {
  const queryText = `
    ...
    CREATE TRIGGER IF NOT EXISTS trigger_update_questions_count_on_insert
    AFTER INSERT
    ON public.questions
    FOR EACH ROW
    EXECUTE FUNCTION update_questions_count_on_insert();

    CREATE TRIGGER IF NOT EXISTS trigger_update_questions_count_on_delete
    AFTER DELETE
    ON public.questions
    FOR EACH ROW
    ...
    CREATE TRIGGER IF NOT EXISTS trigger_increment_voter_count
    AFTER INSERT
    ON public.votes
    FOR EACH ROW
    EXECUTE FUNCTION increment_voter_count();

    CREATE TRIGGER IF NOT EXISTS update_voter_count
    AFTER INSERT
    ON public.votes
    FOR EACH ROW
    EXECUTE FUNCTION increment_voter_count();
