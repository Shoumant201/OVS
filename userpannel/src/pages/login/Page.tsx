...
const LoginPage = ({
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.require2FA) {
        ...
        Cookies.set("token", response.data.token, { expires: 30, path: '/' }) // Added path
        router.push(`/${locale}`) // Navigate to localized home
      }
    } catch (err: any) {
  ...
  const registerUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`${tLogin.registrationSuccessTitle || "Registration Successful!"}\n${tLogin.registrationSuccessMessage || "Please log in."}`)
      setIsLogin(true)
  ...
  const commonInputClass = "bg-gray-100 dark:bg-gray-700 border-0 dark:border-gray-600 h-12 px-4 dark:text-white dark:placeholder-gray-400 focus:ring-2 focus:ring-[#26C6B0] focus:border-[#26C6B0] outline-none";
  const commonButtonClass = "w-full h-12 bg-[#26C6B0] text-white rounded-md font-medium hover:bg-[#20A090] transition-colors disabled:opacity-60 disabled:cursor-not-allowed";
  const ctaButtonClass = "px-8 py-3 bg-white text-[#26C6B0] rounded-full font-medium relative z-10 hover:bg-gray-100 transition-colors"; // Ensure z-index for CTA buttons
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-white dark:bg-black">
          ...
          <>
            ...
            >
              <div className="flex items-center mb-12">
                ...
                <span className="text-lg font-medium text-gray-800 dark:text-gray-200">{tNavbar.title || "Online Voting System"}</span>
              </div>
              ...
              <form onSubmit={handleLogin} className="flex-1 flex flex-col justify-center max-w-md mx-auto w-full">
                  ...
                  {error && (
                    <div className="bg-red-100 border border-red-400 text-red-700 dark:bg-red-900/30 dark:border-red-500 dark:text-red-400 px-4 py-3 rounded mb-4 text-sm" role="alert">{error}</div>
                  )}
                  ...
                  >
                    ...
                    <div className="relative">
                      ...
                      <button
                        ...
                        aria-label={showPassword ? (tMessages.hidePassword || "Hide password") : (tMessages.showPassword || "Show password")}
                      >
                        {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                      </button>
                    </div>
                    <div className="flex justify-end -mt-2">
                      <LocalizedLink href="/forgot-password" className="text-sm text-[#26C6B0] hover:underline dark:text-teal-400">
            ...
            <motion.div
              ...
              transition={{ duration: 0.5 }}
            >
              <div className="absolute inset-0 overflow-hidden z-0"> {/* Ensure patterns are behind content */}
                {/* Background Patterns ... (kept as is for brevity, ensure they don't overlap clickable elements) */}
                <motion.div className="absolute w-64 h-64 rounded-full bg-teal-300/20 -top-10 -right-10" animate={{ scale: [1, 1.2, 1], rotate: [0, 10, 0] }} transition={{ duration: 8, repeat: Infinity, repeatType: "reverse" }} aria-hidden="true" />
                <motion.div className="absolute w-96 h-96 rounded-full bg-teal-300/20 -bottom-20 -left-20" animate={{ scale: [1, 1.1, 1], rotate: [0, -10, 0] }} transition={{ duration: 10, repeat: Infinity, repeatType: "reverse", delay: 1 }} aria-hidden="true" />
                <motion.div className="absolute w-40 h-40 rounded-full bg-teal-300/20 top-1/3 right-1/4" animate={{ scale: [1, 1.3, 1], y: [0, -30, 0] }} transition={{ duration: 12, repeat: Infinity, repeatType: "reverse", delay: 2 }} aria-hidden="true" />
              </div>
                <motion.div
                  initial={{ y: 20, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
          ...
          <>
            ...
            >
              <div className="absolute inset-0 overflow-hidden z-0">
                ...
                >
                  <h2 className="text-3xl font-bold text-white mb-4">{tLogin.alreadyMemberTitle || "Already a Member?"}</h2>
                  <p className="text-white text-opacity-90 mb-8">
            ...
            >
              ...
              <form onSubmit={registerUser} className="flex-1 flex flex-col justify-center max-w-md mx-auto w-full">
                  <motion.div
                    ...
                    className="mb-8"
                  >
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{tLogin.createAccountTitle || "Create Your Account"}</h1>
                    <p className="text-gray-600 dark:text-gray-400">{tLogin.signupUsingSocial || "Sign up using social networks"}</p>
                  </motion.div>
      ...
      <div className="md:hidden fixed bottom-8 right-8 z-20"> {/* Use fixed for mobile button */}
        <motion.button
          type="button"
          onClick={toggleView}
